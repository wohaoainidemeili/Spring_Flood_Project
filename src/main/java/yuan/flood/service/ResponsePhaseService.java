...
@Service
public class ResponsePhaseService implements IResponsePhaseService {
    @Autowired
...
//                System.out.println("yuan:"+sensorObsProperty.getSensorName() + ":" + sensorObsProperty.getPropertyName());
        ...
        try {
            ...
            message.append("您订阅的事件'" + subscibeEventParams.getUserDefineName() + "'于" + dateStr + "进入响应阶段。\r\n");
            message.append("该事件的" + subscibeEventParams.getResponseSensor() + "传感器观测值ֵ");
            message.append("在" + subscibeEventParams.getResponseDay() + "天");
            message.append(subscibeEventParams.getRecoveryHour() + "时");
            message.append(subscibeEventParams.getResponseMinute() + "分");
            message.append(subscibeEventParams.getResponseSecond() + "秒");
            message.append("内大于" + subscibeEventParams.getResponseThreshold() + "m");
